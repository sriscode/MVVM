...
class BooksCatalogViewModel {
    ...
    var showAlert: (() -> ())?
    var gotoDetails: ((BookCatalogDetailsViewModel) -> ())?
    ...
    // on tapping the cell, we are letting the View Controller load the Details View Controller by passing in the BookCatalog object. To avoid View Controller worrying about the navigation, we can implement MVVM - C architectural pattern!
    var selectedIndexPathForBookDetails: IndexPath? {
        didSet {
            ...
            self.gotoDetails?(BookCatalogDetailsViewModel(bookCatalog: booksCatalogs[row]))
        }
    }
